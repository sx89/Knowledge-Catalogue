...
- [panic recover err](#panic-recover-err)
    ...
    - [解析json的方法](#解析json的方法)
        ...
        - [不知道数据结构的时候解析json](#不知道数据结构的时候解析json)
    - [转型包 strconv  ParseXxx](#转型包-strconv--parsexxx)
    - [time](#time)
    - [go一个协程的原理,main怎么办](#go一个协程的原理main怎么办)
    - [http的知识](#http的知识)
        - [请求头](#请求头)
        - [请求行](#请求行)
        - [请求实体](#请求实体)
    - [go实现http请求](#go实现http请求)
        - [http.Get和http.Post](#httpget和httppost)
    ...
    - [nohup与&](#nohup与)
        ...
        - [nohup与&区别](#nohup与区别)
    - [杀掉进程](#杀掉进程)
    - [遍历mysql某个表的所有行](#遍历mysql某个表的所有行)
    - [遍历结构体,切片,map](#遍历结构体切片map)
- [线上数据库的分批查询](#线上数据库的分批查询)
- [Cron表达式(定时格式)](#cron表达式定时格式)
    - [* 星号](#-星号)
    - [? 问号](#-问号)
    - [, 逗号](#-逗号)
    - [斜杠 (/)](#斜杠-)
    ...
    - [W 字母](#w-字母)
    - [井号](#井号)
- [go 中while和do while的实现](#go-中while和do-while的实现)
    - [while](#while)
    - [do while](#do-while)
- [指针](#指针)
    - [重要知识点](#重要知识点)
...
- [值类型与引用类型的区别](#值类型与引用类型的区别)
    - [声明变量时候的却别](#声明变量时候的却别)
    - [数值传递上的区别](#数值传递上的区别)
    - [在new 和 make的区别](#在new-和-make的区别)
        - [new](#new-1)
        - [make](#make-1)
        - [new 和 make的区别](#new-和-make的区别)
- [slice](#slice)
    - [介绍](#介绍)
    - [扩容](#扩容)
    - [nil切片和空切片](#nil切片和空切片)
    - [切片的三种定义方式,map两种](#切片的三种定义方式map两种)
        - [1.定义一个切片，然后让切片去引用一个已经创建好的数组](#1定义一个切片然后让切片去引用一个已经创建好的数组)
        - [2.通过make来创建切片。](#2通过make来创建切片)
        - [3.定义一个切片，直接就指定具体数组，使用原理类似于make的方式](#3定义一个切片直接就指定具体数组使用原理类似于make的方式)
        - [第一种和第二种的区别](#第一种和第二种的区别)
        - [共享数组产生bug](#共享数组产生bug)
- [map](#map-1)
    - [创建](#创建)
    - [底层原理](#底层原理)
...
- [协程](#协程)
    - [原理](#原理)
    - [改变线程多路复用的尴尬](#改变线程多路复用的尴尬)
    - [特点](#特点)
        - [内存占用少](#内存占用少)
        - [非抢占(非公平)](#非抢占非公平)
        - [切换开销小](#切换开销小)
    - [主从函数并发](#主从函数并发)
- [strings.NewReader()](#stringsnewreader)
    - [Len,Size，Read方法](#lensizeread方法)
- [wait group](#wait-group)
    - [WaitGroup对象不是一个引用类型](#waitgroup对象不是一个引用类型)
- [闭包](#闭包-1)
...
- [Go Module](#go-module)
    ...
    - [移除依赖](#移除依赖)
    - [查看依赖包](#查看依赖包)
- [go build,install,get区别](#go-buildinstallget区别)
    - [go build](#go-build)
    - [go install](#go-install)
- [context  源码阅读](#context--源码阅读)
    - [简介](#简介)
    - [源码](#源码)
    - [context的根源上下文](#context的根源上下文)
    - [其他类型的context](#其他类型的context)
    - [WithCancel源码阅读](#withcancel源码阅读)
    - [golang的redis包](#golang的redis包)
        - [redigo模块的详细使用](#redigo模块的详细使用)
            - [连接](#连接)
            - [使用Pipelining操作redis](#使用pipelining操作redis)
...
结构体类型是值类型,拷贝的时候是值拷贝,p2=p1的时候 p1的空间会被拷贝一份;如果修改p1的内容,p2内容不变,如果 var p2 *Person = &p1 这个时候拷贝的不再是内存,而是空间地址,p1,p2指向的是同一个对象

值类型在声明的时候,内部的成员变量就有初始值,赋值不会出现空指针异常
...
<img src="pictures/Golang/image-20200402151757365.png" alt="image-20200402151757365" style="zoom:33%;" />
...
线程等于堆栈资源和程序计数器(装字节码执行进度) 

协程是多个协程存储在一个线程中,协程的堆资源在线程中,栈资源也用线程的堆来保存.所以多个协程可以在一个线程里面运行.

### MPG模式

多个协程G对应多份上下文P,都由一个M来维护.当某个协程G0需要阻塞的时候.就把P0拿下来,换P1上去,继续用M来执行

### 切换模式开销

线程切换时,因为线程的调度是在内核态运行的，而线程中的代码是在用户态运行,会导致切换开销.协程切换在线程内部完成,不会导致模式切换.

### io的优化

golang 对各种 io函数 进行了封装，内部调用操作系统的异步 io函数，当这些异步函数返回 busy 或 bloking 时，golang 利用这个时机将现有的执行序列压栈，让线程去拉另外一个协程的代码来执行，利用并封装了操作系统的异步函数。包括 linux 的 epoll、select.

epoll的优势,文件描述符数量限制更大,零拷贝技术(内核态和用户态有一块共享内存).
