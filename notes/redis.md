...
<!-- TOC -->autoauto- [1. 概述](#1-概述)auto- [2. 数据类型](#2-数据类型)auto    - [2.1. STRING](#21-string)auto    - [2.2. LIST](#22-list)auto    - [2.3. SET](#23-set)auto    - [2.4. HASH](#24-hash)auto    - [2.5. ZSET](#25-zset)auto    - [2.6. 五种基本类型的底层实现TODO](#26-五种基本类型的底层实现todo)auto- [3. 数据结构](#3-数据结构)auto    - [3.1. 字典](#31-字典)auto    - [3.2. 跳跃表](#32-跳跃表)auto- [4. 使用场景](#4-使用场景)auto    - [4.1. 计数器](#41-计数器)auto    - [4.2. 缓存](#42-缓存)auto    - [4.3. 查找表](#43-查找表)auto    - [4.4. 消息队列](#44-消息队列)auto    - [4.5. 会话缓存](#45-会话缓存)auto    - [4.6. 分布式锁实现](#46-分布式锁实现)auto    - [4.7. 其它](#47-其它)auto- [5. Redis 与 Memcached](#5-redis-与-memcached)auto    - [5.1. 数据类型](#51-数据类型)auto    - [5.2. 数据持久化](#52-数据持久化)auto    - [5.3. 分布式](#53-分布式)auto    - [5.4. 内存管理机制](#54-内存管理机制)auto    - [5.5. 缓存过期机制](#55-缓存过期机制)auto- [6. 数据淘汰策略(如何保证Redis中的数据都是热点数据)](#6-数据淘汰策略如何保证redis中的数据都是热点数据)auto    - [Redis 具体有 6 种淘汰策略：](#redis-具体有-6-种淘汰策略)auto    - [Redis过期key清除策略](#redis过期key清除策略)auto- [Redis过期细节](#redis过期细节)auto    - [清除过期时间](#清除过期时间)auto    - [Redis键值2种失效方式](#redis键值2种失效方式)auto- [7. 持久化](#7-持久化)auto    - [7.1. RDB 持久化](#71-rdb-持久化)auto        - [rdb 的bgsave 很占用内存](#rdb-的bgsave-很占用内存)auto        - [rdb的原理](#rdb的原理)auto        - [写时复制的原理](#写时复制的原理)auto    - [7.2. AOF 持久化](#72-aof-持久化)auto    - [AOF 重写](#aof-重写)auto    - [7.3. Redis两种持久化方案对比](#73-redis两种持久化方案对比)auto- [8. 事务](#8-事务)auto    - [8.1. redis事务相关命令](#81-redis事务相关命令)auto    - [8.2. redis数据库事务的ACID与传统关系型事务的比较](#82-redis数据库事务的acid与传统关系型事务的比较)auto        - [8.2.1. 原子性（Atomicity）](#821-原子性atomicity)auto        - [8.2.2. 一致性（Consistency）](#822-一致性consistency)auto        - [8.2.3. 隔离性（Isolation）](#823-隔离性isolation)auto        - [8.2.4. 持久性（Durability）](#824-持久性durability)auto    - [10.3. redis主从集群的搭建与docker通信知识](#103-redis主从集群的搭建与docker通信知识)auto        - [10.3.1. docker搭建redis主从集群](#1031-docker搭建redis主从集群)auto        - [10.3.2. docker 搭建redis-sentinel集群](#1032-docker-搭建redis-sentinel集群)auto- [11. 订阅机制](#11-订阅机制)auto    - [11.1. redis的消息订阅发布和mq对比？](#111-redis的消息订阅发布和mq对比)auto- [12. redis作为二级缓存](#12-redis作为二级缓存)auto    - [12.1. 实现](#121-实现)auto    - [12.2. springboot cache自定义项](#122-springboot-cache自定义项)auto    - [12.3. mysql与redis性能对比](#123-mysql与redis性能对比)auto- [13. 缓存穿透与缓存雪崩](#13-缓存穿透与缓存雪崩)auto    - [13.1. 缓存穿透](#131-缓存穿透)auto    - [13.2. 缓存雪崩](#132-缓存雪崩)auto    - [13.3. 缓存雪崩解决方案](#133-缓存雪崩解决方案)auto- [14. redis实现分布式锁(解决不同客户端并发操作某个数据)](#14-redis实现分布式锁解决不同客户端并发操作某个数据)auto    - [14.1. 分布式锁是什么](#141-分布式锁是什么)auto    - [14.2. 分布锁实现方案分析](#142-分布锁实现方案分析)auto        - [14.2.1. 分布式锁面试题](#1421-分布式锁面试题)auto- [15. redis高可用集群](#15-redis高可用集群)auto    - [15.1. 主从复制的原理](#151-主从复制的原理)auto    - [15.2. 分布式系统高可用](#152-分布式系统高可用)auto    - [数据迁移的三种方式](#数据迁移的三种方式)auto    - [15.3. Redis高可用架构Sentinel(解决宕机)](#153-redis高可用架构sentinel解决宕机)auto        - [15.3.1. Sentinel三大工作任务](#1531-sentinel三大工作任务)auto        - [15.3.2. 冷备和热备](#1532-冷备和热备)auto        - [15.3.3. Sentinel故障转移原理](#1533-sentinel故障转移原理)auto        - [15.3.4. sentinel整合Springboot实战](#1534-sentinel整合springboot实战)auto    - [15.4. Redis内置高可用集群](#154-redis内置高可用集群)auto        - [15.4.1. Redis集群搭建](#1541-redis集群搭建)auto        - [15.4.2. 一致性Hash算法](#1542-一致性hash算法)auto            - [一致性hash的特性](#一致性hash的特性)auto        - [15.4.3. 一致性Hash算法虚拟节点](#1543-一致性hash算法虚拟节点)auto        - [15.4.4. twemproxy实现hash分片](#1544-twemproxy实现hash分片)auto- [16. 布隆过滤器](#16-布隆过滤器)auto        - [16.0.5. 介绍](#1605-介绍)auto        - [16.0.6. 使用场景](#1606-使用场景)auto    - [16.1. 过滤器原理](#161-过滤器原理)auto    - [16.2. goole布隆过滤器与Redis布隆过滤器](#162-goole布隆过滤器与redis布隆过滤器)auto    - [布隆过滤器扩容](#布隆过滤器扩容)auto    - [16.3. Redis布隆过滤器安装](#163-redis布隆过滤器安装)auto    - [16.4. Redis布隆过滤器与springboot的整合探索](#164-redis布隆过滤器与springboot的整合探索)auto- [redis的并发竞争问题(修改一个值,商品超卖)](#redis的并发竞争问题修改一个值商品超卖)auto    - [解决方案](#解决方案)auto        - [分布式锁](#分布式锁)auto            - [数据库实现方式](#数据库实现方式)auto            - [zookeeper实现](#zookeeper实现)auto            - [redis实现](#redis实现)auto        - [利用消息队列](#利用消息队列)auto        - [用事务或者lua脚本实现乐观锁](#用事务或者lua脚本实现乐观锁)auto        - [其他想法](#其他想法)auto- [redis与数据库的一致性(缓存入库万一mysql失败了怎么办)](#redis与数据库的一致性缓存入库万一mysql失败了怎么办)auto    - [读缓存时](#读缓存时)auto    - [写缓存时](#写缓存时)auto        - [为什么是删除缓存，而不是更新缓存？](#为什么是删除缓存而不是更新缓存)auto    - [高并发的场景下，该解决方案要注意的问题：](#高并发的场景下该解决方案要注意的问题)auto- [RDB原理再理解](#rdb原理再理解)auto    - [rdbSave](#rdbsave)auto    - [save && bgsave](#save--bgsave)auto    - [bgsave](#bgsave)auto    - [服务器的dirty](#服务器的dirty)auto    - [rdbLoad](#rdbload)auto- [lua脚本保证操作原子性](#lua脚本保证操作原子性)auto- [雪花算法](#雪花算法)auto    - [SnowFlake作用](#snowflake作用)auto    - [雪花中的溢出问题](#雪花中的溢出问题)auto    - [雪花中的冬令时问题(时钟回拨问题)](#雪花中的冬令时问题时钟回拨问题)auto- [面试题](#面试题)auto    - [修改配置不重启Redis会实时生效吗？](#修改配置不重启redis会实时生效吗)auto    - [Redis的内存用完了会发生什么？](#redis的内存用完了会发生什么)auto    - [redis常见性能问题和解决方案：](#redis常见性能问题和解决方案)autoauto<!-- /TOC -->
